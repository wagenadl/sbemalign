...
# I'll call the table MONTAGEALIGNQ5X.
...
# from MONTAGEALIGNQ25Y.
# Note that dx,dy are inclusive of the baseshift from MONTAGEALIGNQ25Y.
...
def droptable():
    db.exe('''drop table montagealignq5x''')

def maketable():
    db.exe('''create table if not exists montagealignq5x (
    r integer,
    m integer,
    s integer,
    ix integer,
...
def alignsubtiles(r, m, s, ix, iy, tileimg, neighborhoodimg):
    ...
    db.exe(f'''insert into montagealignq5x 
    (r,m,s,ix,iy,
...
def alignmanysubtiles(r, m, ix, iy):
    cnt = db.sel(f'''select count(1) from montagealignq5x
    ...
    def loader(subtileid):
        ...
        if tileid in baseshifts:
            ...
            dx,dy = db.sel(f'''select dx+dxb,dy+dyb from montagealignq25y
    ...
    if cnt>0:
        db.exe(f'''delete from montagealignq5x 
