...
This FILLS the UCTSHIFT0 table in the db with (r,m,s) and (dx,dy,sx,sy,snr)
data. A given dx value means that the estimated true center position of a
tile is betapos.xc + uctshift.dx.
This version uses the per-run average of the betapos data, and stores (xc,yc)
averages in the UCTSHIFT0 table.
This also FILLS the AVGBETAPOS table.
...
exe('''create table if not exists uctshift0 (
...
def aligntile(uct, r, m, s):
    ...
    xc, yc = sel('''select xc0, yc0 from avgbetapos
    where r=%s and m=%s''',
                 (r, m))[0]
    Y,X = em.shape
    Rem = 512
    ...
    return (cdx, cdy, csx, csy, snr, iter, xc, yc)
...
def doslice(r, s):
    ...
    for m in range(nmont[r]):
        if m not in mm:
            (dx, dy, sx, sy, snr, iter, xc, yc) = aligntile(uct, r, m, s)
            exe('''insert into uctshift0 (r,m,s, xc0,yc0, dx,dy,sx,sy, snr,iter)
            values (%s,%s,%s, %s, %s, %s,%s,%s,%s,%s,%s)''',
                (r,m,s, float(xc), float(yc),
...
exe('create table if not exists avgbetapos as select r,m,sum(xc)/sum(1) as xc0,sum(yc)/sum(1) as yc0 from betapos group by r,m order by r,m')

for r in z0.keys():
    ndone = sel('select count(*) from uctshift0 where r=%s', (r,))[0][0]
