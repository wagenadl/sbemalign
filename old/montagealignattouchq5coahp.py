...
def droptable():
    db.exe('''drop table montagealignattouchq5coahp''')

def maketable():
 
    db.exe('''create table if not exists montagealignattouchq5coahp (
    r integer,
    m integer,
    s integer,
    ix integer,
...
def alignsubtiles(subtileid, row, tileimg, neighborhoodimg):
    ...
    if neighborhoodimg is None:
        db.exe(f'''insert into montagealignattouchq5coahp 
        (r,m,s,ix,iy,x,y,m2,
...
def alignmanysubtiles(r, m, ix, iy):
    def loader(subtileid):
        ...
        if subtileid in baseshifts:
            ...
            dx,dy = db.sel(f'''select dx+dxb,dy+dyb from montagealignq5coahp
    ...
    db.exe(f'''delete from montagealignattouchq5coahp 
...
def queuealignmanysubtiles(r, m, ix, iy):
    cnt = db.sel(f'''select count(1) from montagealignattouchq5coahp 
