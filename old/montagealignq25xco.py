...
def droptable():
    db.exe('''drop table montagealignq25xco''')

def maketable():
    db.exe('''create table if not exists montagealignq25xco (
    r integer,
    m integer,
    s integer,
...
def aligntiles(r, m, s, tileimg, neighborhoodimg):
    if neighborhoodimg is None:
        db.exe(f'''insert into montagealignq25xco 
        (r,m,s,
...
def alignmontage(r, m):
    ...
    db.exe(f'''delete from montagealignq25xco where r={r} and m={m}''')
    ifns = []
    for s in range(ri.nslices(r)):
        tileid = (r,m,s)
        ifns.append(tileid)
    swiftir.buildout(ifns, loader=loader, saver=saver, nbase=21)

fac = factory.Factory(nthreads)
    
def queuealignmontage(r, m):
    cnt = db.sel(f'''select count(1) from montagealignq25xco 
