...
# The dx here is inclusive of the shift in MONTAGEALIGNQ25X.
# I'll call the table MONTAGEALIGNQ25Y.
...
def droptable():
    db.exe('''drop table montagealignq25y''')

def maketable():
    db.exe('''create table if not exists montagealignq25y (
    r integer,
    m integer,
    s integer,
...
def aligntiles(r, m, s, tileimg, neighborhoodimg):
    ...
    dx0,dy0 = db.sel(f'''select dx+dxb,dy+dyb from montagealignq25x
    where r={r} and m={m} and s={s}''')[0]
    dx += dx0
    dy += dy0
    
    db.exe(f'''insert into montagealignq25y 
    (r,m,s,
...
def alignmontage(r, m):
    def loader(tileid):
        r,m,s = tileid
        dx,dy = db.sel(f'''select dx+dxb,dy+dyb from montagealignq25x
        where r={r} and m={m} and s={s}''')[0]
        try:
    ...
    db.exe(f'''delete from montagealignq25y where r={r} and m={m}''')
    subtileids = []
...
def queuealignmontage(r, m):
    cnt = db.sel(f'''select count(1) from montagealignq25y 
