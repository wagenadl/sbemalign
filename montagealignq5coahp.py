...
# This version applies high-pass filtering to the Q25XCO data
...
def droptable():
    db.exe('''drop table montagealignq5coahp''')

def maketable():
    db.exe('''create table if not exists montagealignq5coahp (
    r integer,
    m integer,
    s integer,
    ix integer,
...
def alignsubtiles(r, m, s, ix, iy, tileimg, neighborhoodimg):
    ...
    if neighborhoodimg is None:
        db.exe(f'''insert into montagealignq5coahp 
        (r,m,s,ix,iy,
...
def getbaseshifts(r, m):
    s,dx,dy = db.vsel(f'''select s,dx+dxb,dy+dyb from montagealignq25xco
            where r={r} and m={m} order by s''')
    dx *= 5
    dy *= 5
    S = len(s)
    if S != ri.nslices(r):
        raise Exception(f'Bad number of slices for R{r} M{m}')
    s0 = S//2
    dxf = dx.copy()
    dyf = dy.copy()
    LAMBDA = 25
    ...
    for s in range(s0-1,-1,-1):
        ...
        dyf[s] = fac * (dyf[s+1] + dy[s] - dy[s+1])
    for s in range(S):
        baseshifts[(r,m,s)] = (dxf[s],dyf[s])
    
def alignmanysubtiles(r, m, ix, iy):
    cnt = db.sel(f'''select count(1) from montagealignq5coahp
    ...
    if cnt>0:
        db.exe(f'''delete from montagealignq5coahp 
